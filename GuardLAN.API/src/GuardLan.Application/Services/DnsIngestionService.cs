    ...
    public async Task<DnsIngestionResultDto> ImportRecentAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (!dnsQuerySource.IsEnabled)
        {
            return new DnsIngestionResultDto(
                ...
                MatchedDevices: 0,
                importedAtUtc,
                $"{dnsQuerySource.SourceName} ingestion is disabled.");
        }
        ...
        if (sourceRecords.Count == 0)
        {
            return new DnsIngestionResultDto(
                ...
                MatchedDevices: 0,
                importedAtUtc,
                $"No DNS records were returned by {dnsQuerySource.SourceName}.");
        }
        ...
        if (validRecords.Length == 0)
        {
            return new DnsIngestionResultDto(
                ...
                MatchedDevices: 0,
                importedAtUtc,
                $"No valid DNS records were returned by {dnsQuerySource.SourceName}.");
        }
        ...
        return new DnsIngestionResultDto(
            ...
            matchedDevices,
            importedAtUtc,
            $"Imported {imported} DNS records from {dnsQuerySource.SourceName}.");
    }
